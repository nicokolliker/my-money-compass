...
export function useTransactions(filters?: { accountId?: string; categoryId?: string; type?: string; search?: string; dateFrom?: string; dateTo?: string }) {
  ...
  return useQuery({
    ...
    enabled: !!userId,
    queryFn: async () => {
      let q = supabase.from('transactions').select('*, accounts(name, currency), categories(name, icon, color), merchants(id, name, display_name, logo_url, default_category_id)').order('date', { ascending: false }).order('created_at', { ascending: false });
